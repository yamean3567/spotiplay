    ...
    return (
        <div className="flex min-h-screen bg-gradient-to-b from-black via-black to-green-900 text-gray-300 font-mono flex-col md:flex-row">
                ...
                <div className="flex-1 text-center gap-y-2 grid-cols-3 grid md:h-24 md:grid-cols-1 md:gap-y-1 md:mt-10">
                    ...
                    <div className="mb-4 grid justify-items-center mx-auto border-2 rounded-lg border-green-800 w-24 h-16 md:h-22 text-sm md:text-2xl py-2 md:w-36">
                            {props.gameTime}s <br/>
                            <FcAlarmClock size={24} />
                    </div>
                    <div className="mb-4 text-center mx-auto border-2 rounded-lg w-24 h-16 md:h-22 border-green-800 text-sm py-2 md:text-2xl md:w-36">
                    {props.currentScore} <br/>
                            pts
                    </div>
                    {loading && 
                        <div className="mb-4 decoration-clone mx-auto bg-gradient-to-b from-green-900 via-green-800 to-green-400 animate-pulse text-center border-2 rounded-lg border-black text-sm w-24 h-16 md:h-22 md:text-2xl py-2 md:w-36">
                            +{props.score}
                        </div>}
                </div>
        </div>
