    ...
    return (
        <div className="flex flex-col bg-gradient-to-b from-black via-black to-green-900 text-gray-300 h-screen font-mono">
            ...
            <div className='flex flex-col items-center gap-y-2'>
                    <button className="md:mb-5 text-center p-3 border-2 rounded-lg h-22 border-green-800 hover:bg-green-600 text-sm font-bold text-white"
                     disabled={disabled} onClick={() => {props.restartGame(); setDisabled(true);}}>{!disabled ? "Restart" : "Restarting in..." + props.time}</button>
                    <button className="md:mb-5 p-3 text-center border-2 rounded-lg h-22 border-gray-800 hover:bg-gray-600 text-sm font-bold text-white"
                     onClick={() => props.navigate("/home")}>Exit</button>
            </div>
        </div>
    )
}
